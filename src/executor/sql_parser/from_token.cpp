...
Expected<ITableInput> TFromToken::Exec() {
    for (const auto& t : args_) {
        if (t->GetType() != ETokens::ENameToken) {
    ...
    if (args_.size() == 1) {
        ...
        TIOFactory::RegisterFileIO(scheme, ETypeFile::ECSVFile);
        TIOFactory::RegisterFileIO(data, ETypeFile::ECSVFile);
