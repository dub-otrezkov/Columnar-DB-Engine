...
clickhouse start 2>/dev/null || true
sleep 2

echo "Creating table..."
clickhouse-client --multiquery < "$DIR/create_hits.sql"

echo "Loading data..."
clickhouse-client --max_threads=1 --query "INSERT INTO hits FORMAT CSVWithNames" < "$CSV"
echo "Loaded $(clickhouse-client --query 'SELECT COUNT(*) FROM hits') rows"
...
declare -a queries=(
    ...
    "SELECT UserID, toMinute(EventTime) AS m, SearchPhrase, COUNT(*) AS c FROM hits GROUP BY UserID, m, SearchPhrase ORDER BY c DESC LIMIT 10"
    ...
    $'SELECT replaceRegexpAll(Referer, \'^https?://(?:www\\.)?([^/]+)/.*$\', \'\\1\') AS k, AVG(length(Referer)) AS l, COUNT(Referer) AS c, MIN(Referer) FROM hits WHERE Referer <> \'\' GROUP BY k HAVING c > 100000 ORDER BY l DESC LIMIT 25'
    ...
    "SELECT 1, URL, COUNT(*) AS c FROM hits GROUP BY URL ORDER BY c DESC LIMIT 10"
    ...
    "SELECT TraficSourceID, SearchEngineID, AdvEngineID, if(SearchEngineID = 0 AND AdvEngineID = 0, Referer, '') AS Src, URL AS Dst, COUNT(*) AS PageViews FROM hits WHERE CounterID = 62 AND EventDate >= '2013-07-01' AND EventDate <= '2013-07-31' AND IsRefresh = 0 GROUP BY TraficSourceID, SearchEngineID, AdvEngineID, Src, Dst ORDER BY PageViews DESC LIMIT 10 OFFSET 1000"
    "SELECT URLHash, EventDate, COUNT(*) AS PageViews FROM hits WHERE CounterID = 62 AND EventDate >= '2013-07-01' AND EventDate <= '2013-07-31' AND IsRefresh = 0 AND TraficSourceID IN (-1, 6) AND RefererHash = 3594120000172545465 GROUP BY URLHash, EventDate ORDER BY PageViews DESC LIMIT 10 OFFSET 100"
    "SELECT WindowClientWidth, WindowClientHeight, COUNT(*) AS PageViews FROM hits WHERE CounterID = 62 AND EventDate >= '2013-07-01' AND EventDate <= '2013-07-31' AND IsRefresh = 0 AND DontCountHits = 0 AND URLHash = 2868770270353813622 GROUP BY WindowClientWidth, WindowClientHeight ORDER BY PageViews DESC LIMIT 10 OFFSET 10000"
    "SELECT toStartOfMinute(EventTime) AS M, COUNT(EventTime) AS PageViews FROM hits WHERE CounterID = 62 AND EventDate >= '2013-07-14' AND EventDate <= '2013-07-15' AND IsRefresh = 0 AND DontCountHits = 0 GROUP BY M ORDER BY M LIMIT 10 OFFSET 1000"
)
...
for q in "${queries[@]}"; do
    start=$(date +%s%3N)
    clickhouse-client --max_threads=1 --query "$q" > /dev/null
    end=$(date +%s%3N)
    elapsed=$((end - start))
    total=$((total + elapsed))
    printf "%-15s | %.80s\n" "${elapsed}" "$q"
...
printf '%0.s=' {1..100}; echo ""
echo "SUMMARY:"
echo "Total execution time: ${total} ms"
